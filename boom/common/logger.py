...
from boom.common import TASK_SET
...
class Logger:
    ...
    def __init__(self, cfg):
        ...
        self.wandb_savefolder = self._wandb.run.dir
        print(colored(f"Wandb run directory: {self.wandb_savefolder}", "blue", attrs=["bold"]))
        self.output_file = self.wandb_savefolder + "/eval.txt"
    ...
    def _print(self, d, category):
        ...
        for k, disp_k, ty in CONSOLE_FORMAT:
            if k in d:
                pieces.append(f"{self._format(disp_k, d[k], ty):<16}")
        print(" ".join(pieces))
